  ...
  return (
    <div className="mx-auto p-5">
      ...
      <h3 className="text-xl">
        <span className="font-bold">Ans:</span> Access token is a token which is
        used in token-based authentication to allow an application to access an
        API and the refresh token is used to generate a new access token.
      ...
      <h3 className="text-xl">
        <span className="font-bold">Ans:</span> SQL databases are vertically
        scalable, while NoSQL databases are horizontally scalable. SQL databases
        are table-based, while NoSQL databases are document, key-value, graph,
        or wide-column stores. SQL databases are better for multi-row
        transactions, while NoSQL is better for unstructured data like documents
        or JSON.{" "}
      </h3>
      ...
      <h3 className="text-xl">
        ...
        passing them through different stages. The stages make up what is known
        as a pipeline. The stages in a pipeline can filter, sort, group, reshape
        and modify documents that pass through the pipeline.The input of the
        pipeline can be a single collection, where others can be merged later
        down the pipeline. The pipeline then performs successive transformations
        on the data until our goal is achieved. This way, we can break down a
        complex query into easier stages, in each of which we complete a
        different operation on the data. So, by the end of the query pipeline,
        we will have achieved all that we wanted.
      </h3>
    </div>
  );
};
